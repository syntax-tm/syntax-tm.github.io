...
const ChromaEgg = (props?: EggIconProps) => {
  return (
    <svg
      aria-hidden="true"
      className={`svg-inline--fa fa-egg ${props?.className ?? ''}`}
      data-icon="egg"
      data-prefix="fas"
      viewBox="0 0 384 512"
      width={props?.width}
...
export function EggIcon(props?: EggIconProps) {
  ...
  const { currentItemType } = useXmb();

  const className = props?.className ?? '';

  const isSelected = currentItemType === 'secrets';
  const nonSelectedComponent = useMemo(() => {
    return <FontAwesomeIcon icon={faEgg} width={120} height={120} className={`xmb-icon text-rose-500! drop-shadow-sm drop-shadow-black/60 ${className}`} />;
  }, [className]);

  return isSelected
    ? <ChromaEgg className={className} width={120} height={120} />
